...
const Monarch const* glth::io::get_monarch_ptr() const {
  return (this->_monarch_ptr);
}

const Monarch const* glth::io::get_monarch_ptr() {
  return static_cast<const io*>(this)->get_monarch_ptr();
}
...
glth_const::io_result glth::io::populate(signal* ch1, signal* ch2) {
  ...
  if( (ch1->size()) != (this->get_record_length()) 
      || (ch2->size()) != (this->get_record_length()) ) {
  ...
  // OK
    ...
    if( this->_monarch_ptr->ReadRecord() == true ) {
      ...
      work_rec = (this->_monarch_ptr->GetRecord());
	...
	double pt = 0;
	ch1->at(idx) = glth_px1500::min_mvolts + 
	  (glth_px1500::max_mvolts)*(work_rec->fDataPtr[idx])/(2<<glth_px1500::n_bits);
      }
	...
	for(std::size_t idx = 0; idx < ch2->size(); idx++) {
	  double pt = 0;
	  ch2->at(idx) = glth_px1500::min_mvolts + 
	    (glth_px1500::max_mvolts)*(work_rec->fDataPtr[idx])/(2<<glth_px1500::n_bits);
	}
      }
