...
int main(int argc, char** argv) {
  ...
  if( env::verify(*env) != env_success ) {
	...
	nbins = 128;
      ...
      std::size_t wvd_coarse_stride = (std::size_t)(5.0e-6*(2.0*nyquist_f*1.0e6));
      std::cout << "**Coarse time stride is " << wvd_coarse_stride << std::endl;
	...
	std::cout << "**Calculating XWVD at predetermined slices..." << std::endl;
	for(std::size_t i = 1024; i < record_len; i += wvd_coarse_stride) {
	  xfm.xwvd_slice(ch1,ch2,&tfr,i);
	}
